---
name: engram-save
description: Save a NEW note to the user's Obsidian vault knowledge base. Trigger any time the user wants to capture and preserve something for the first time — a fix they just figured out, an architecture decision, setup steps, a useful command, a protocol, or anything they say they "don't want to forget" or want "future me" to know. Key phrases: "save this", "remember this", "add to vault", "engram this", "save as engram", "keep a note about", "document this". Only trigger when the user explicitly asks to save to engram/vault, or when knowledge is clearly cross-project and not repo-specific. In repo context, prefer /context-doc for repo-local docs. Do NOT trigger for: finding or recalling existing notes (→ engram-search), modifying or appending to an existing note (→ engram-update), or browsing vault contents (→ engram-browse).
